...
> Answer: We can use `ls` to see if files and folders belong to the correct directory.
...
> The output goes into the file instead of just outputing the text in the Terminal.
...
> What changed?
> Size of files is displayed in a different format (it uses common abbreviations instead of bytes).
